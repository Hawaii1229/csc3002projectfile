    ...
    ui(new Ui::thirdwindow_toc)
{
    ...
    // 连接数据库
    m_dbbill = QSqlDatabase::addDatabase("QSQLITE", "bill_connection");
    ...
    if (!query.exec(QString("SELECT name, price, quantity FROM %1 WHERE quantity > 0").arg(m_strStoreName))) {
        ...
        while (query.next()) {
            ...
            ui->bill->insertRow(t_iRowx); // 在该行后面添加一行
            // new 几个表格item填入数据
...
void thirdwindow_toc::setPayment(int t_i)
{
    ...
    ui->bill->setItem(t_iRowx, 2, new QTableWidgetItem("Delivery Fee:"));
    ...
    ui->bill->insertRow(t_iRowx);
    ui->bill->setItem(t_iRowx, 2, new QTableWidgetItem("Total:"));
...
void thirdwindow_toc::on_pushButton_clicked()
{
    ...
    QMessageBox::information(this, "Pay successful", "Return to main window");
    if (query.exec()) {
          emit closeSecondwindow(); // 确保在成功执行后发射信号
    }
    this->close(); // 发送关闭选餐窗口的信号
}